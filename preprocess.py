...
def build_vocab(args):
    ...
    f = open(args.embed)
    embed_dim = len(next(f).split()[1:])

    word2id = OrderedDict()
    
    word2id[PAD_TOKEN] = PAD_IDX
    word2id[UNK_TOKEN] = UNK_IDX
    word2id[SOS_TOKEN] = SOS_IDX
    word2id[EOS_TOKEN] = EOS_IDX
    word2id[SSOS_TOKEN] = SSOS_IDX
    word2id[SEOS_TOKEN] = SEOS_IDX
...
if __name__ == '__main__':
    ...
    parser.add_argument('-source_dir', type=str, default='data/train_ex.json')
    parser.add_argument('-target_dir', type=str, default='data/val_ex.json')
